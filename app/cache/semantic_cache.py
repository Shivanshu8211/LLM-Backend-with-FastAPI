...
class InMemoryCacheStore:
    ...
    def keys_latest(self, limit: int) -> list[str]:
        with self._lock:
            return list(reversed(self._order[-max(1, limit) :]))
...
class RedisSemanticCache:
    ...
    def _evict_if_needed(self) -> None:
        ...
        if self._redis:
            count = self._redis.zcard(self._zset_key())
            if count <= max_entries:
                return
            drop = count - max_entries
            keys = self._redis.zrange(self._zset_key(), 0, max(0, drop - 1))
            for key in keys:
                self._delete_key(key)
            return
        count = self._memory.size()
        if count <= max_entries:
            return
        drop = count - max_entries
        oldest = list(reversed(self._memory.keys_latest(limit=count)))[:drop]
        for key in oldest:
            self._delete_key(key)
    ...
    def lookup(self, prompt: str, allow_semantic: bool = True) -> CacheLookupResult:
        ...
        exact = self._read_record(exact_key)
        if exact and exact.get('output'):
            self._stats.inc('exact_hits')
            return CacheLookupResult(hit=True, output=str(exact['output']), hit_type='exact')
    ...
    def store(self, prompt: str, output: str) -> None:
        ...
        try:
            ...
            if self._redis:
                ...
                self._redis.zadd(self._zset_key(), {key: record['created_at']})
            else:
                self._memory.set(key, record, ttl_seconds=ttl)
    ...
    def clear(self) -> int:
        ...
        if self._redis:
            ...
            for key in list(self._redis.scan_iter(match=pattern)):
                self._delete_key(key)
                removed += 1
    ...
    def status(self) -> dict[str, Any]:
        entries = self._redis.zcard(self._zset_key()) if self._redis else self._memory.size()
        return {
            'enabled': settings.cache_enabled,
            'configured_backend': self._mode,
            'active_backend': 'redis' if self._redis else 'memory',
            'redis_connected': self._redis is not None,
            'redis_url': settings.redis_url if self._mode == 'redis' else None,
